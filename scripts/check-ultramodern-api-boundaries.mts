  ...
  for (const match of source.matchAll(importPattern)) {
    ...
    const names = new Set(
      namedBindings
        .split(',')
        .map(
          (binding) => binding.trim().split(/\s+as\s+/u)[1] ?? binding.trim()
        )
        .filter(Boolean)
    );
...
const apiVerticalDirectories = verticalDirectories.filter((verticalPath) =>
  appShouldEmitApi(configuredApp(verticalPath))
);
...
function assertRpcSurface(appPath, app) {
  ...
  assertNoPath(
    restContract,
    `${appPath} RPC unit must not emit ${restContract}.`
  );
  assertNoPath(
    restClient,
    `${appPath} RPC unit must not emit the REST API client.`
  );
  ...
  assertModuleShape(sharedRpc, {
    ...
    calls: [
      [
        'RpcGroup.make',
        'must declare the RPC group through RpcGroup.make(...).',
      ],
...
function assertApiSurface(appPath, app) {
  ...
  assertNoPath(
    rpcContract,
    `${appPath} REST unit must not emit ${rpcContract}.`
  );
  assertNoPath(
    rpcClient,
    `${appPath} REST unit must not emit the RPC API client.`
  );
  ...
  assertModuleShape(sharedApi, {
    ...
    calls: [
      ...
      [
        'HttpApiGroup.make',
        'must declare groups through HttpApiGroup.make(...).',
      ],
      [
        'HttpApiEndpoint.get',
