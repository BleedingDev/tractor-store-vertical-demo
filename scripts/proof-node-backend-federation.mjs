...
function readBuildIdentity(app) {
  const buildModulePath = path.join(workspaceRoot, app.directory, 'shared/ultramodern-build.ts');
...
function validateManifest(app, manifest, buildIdentity) {
  ...
  assertEqual(manifest.version, buildIdentity.version, `${app.id} backend manifest version`);
  assertEqual(
...
async function proveBackend(app, backendRuntime, target) {
  const { createEffectBffTestHandler, loadBackendFederatedEffectApiFromManifest } = backendRuntime;
