...
const createRemoteManifestUrl = (options: {
  ...
  const configuredManifest = getBuildConfigEnvironment(
    options.manifestEnv
  )?.trim();
  ...
  const configuredPublicUrl = getBuildConfigEnvironment(
...
const pluginI18nVersion = (
  require('@modern-js/plugin-i18n/package.json') as { version: string }
).version;
